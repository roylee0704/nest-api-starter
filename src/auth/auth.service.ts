...
import { EntityNotFoundError, PasswordResetTokenExpiredError, InvalidCredentials, BadRequestError } from '@shared/errors';
...
export class AuthService {
    ...
    async logout(userId: string, userType: UserType): Promise<boolean> {
        const userLogin = await this.repository.findOne({ userId, userType });
        if (!userLogin) {
            throw new BadRequestError();
        }

        this.updateLoginStatus(userLogin, false);
        return true;
    }
